...
class CosmoInterpeter : public InterpreterInterface, public kitBase::DevicesConfigurationProvider
{
	Q_OBJECT
...
public:
	...
	CosmoInterpeter(const qReal::GraphicalModelAssistInterface &graphicalModelApi
...
private:
	...
	void addThread(CosmoThread * const thread, const QString &threadId);
	...
	QHash<QString, CosmoThread *> mThreads;  // Has ownership
	const kitBase::robotModel::RobotModelManagerInterface &mRobotModelManager;
	details::BlocksTable *mBlocksTable;  // Has ownership

	using DataFlowRobotsBlock = dataFlowInterpretation::DataFlowRobotsBlock;
